...
interface LayerControlsProps {
  ...
  onToggleLayer: (layer: 'buildings' | 'ionLine' | 'ionStations' | 'parcelScores') => void;
  onReset: () => void;
  onOpenSettings: () => void;
  ionSimMode: boolean;
  onToggleIonSim: () => void;
    ...
    <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 items-end">
      ...
      <div className="relative flex flex-col items-end">
        <button
          ...
          className={`mb-2 ${btnClass(layersOpen)}`}
        >
        ...
        {layersOpen && (
          <div className="absolute top-0 right-12 w-52 bg-[#111D32] border border-[#1E3050] rounded-xl p-3 shadow-xl flex flex-col gap-3">
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={visibleLayers.parcelScores} onChange={() => onToggleLayer('parcelScores')} className="accent-[#fdd835] cursor-pointer" />
              <span className="text-[13px] text-[#94A3B8] group-hover:text-white transition-colors">Parcel Scores</span>
            </label>
          </div>
        )}
      </div>

      <button onClick={onToggle3D} className={btnClass(is3DMode)}>
        <Box className="w-5 h-5" />
      </button>

      <button onClick={onToggleLight} className={btnClass(isLightMode)}>
      ...
      <button
        onClick={onToggleIonSim}
        className={btnClass(ionSimMode)}
        title="Simulate ION Station"
      >
      ...
      <button
        ...
        className={btnClass(drawAreaMode)}
        title="Draw Area for Analysis"
      >
