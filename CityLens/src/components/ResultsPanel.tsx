...
export default function ResultsPanel({ 
  ...
  return (
    <div 
      className={`absolute top-0 right-0 h-full w-full max-w-[380px] bg-[#050A1A]/95 backdrop-blur-xl border-l border-[#1E3050] z-20 flex flex-col transition-transform duration-400 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      style={{ fontFamily: 'Rubik' }}
    >
      <div className="flex-1 overflow-y-auto hide-scrollbar p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <h2 className="text-[18px] text-white font-semibold flex items-center gap-2" style={{ fontFamily: 'Unbounded' }}>
              Simulation Results
            </h2>
            {isSpeaking && (
          ...
          <button 
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-[#0A0F2E] hover:bg-[#1E3050] text-[#94A3B8] transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col gap-4">
            <div className="h-20 bg-[#0A0F2E] rounded-xl animate-pulse" />
            <div className="flex gap-2 mb-2">
              <div className="h-6 w-20 bg-[#0A0F2E] rounded-full animate-pulse" />
              <div className="h-6 w-32 bg-[#0A0F2E] rounded-full animate-pulse" />
            </div>
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-24 bg-[#0A0F2E] rounded-xl animate-pulse" />
            ))}
          </div>
        ) : result ? (
          <>
            <div className="mb-6 bg-[#0A0F2E] border border-[#1E3050] p-4 rounded-xl leading-relaxed text-[14px] text-white">
              {result.narrative}
            </div>
            ...
            {result.risks && result.risks.length > 0 && (
              <div className="mb-6 flex flex-wrap gap-2">
                {result.risks.map((risk, i) => (
                  <span key={i} className="flex items-center gap-1 bg-[#581C87]/40 border border-[#7E22CE]/50 text-[#D8B4FE] px-2 py-1 rounded text-[11px] font-medium uppercase tracking-wide">
                    <AlertTriangle className="w-3 h-3 flex-shrink-0" /> {risk}
                  </span>
                ))}
              </div>
            )}
            ...
            <div className="flex flex-col gap-4">
              {statConfig.map((config, i) => {
                ...
                return (
                  <div 
                    key={i} 
                    className={`bg-[#0A0F2E] rounded-xl p-4 border transition-colors ${isWarning ? 'border-[#EF4444]/50' : 'border-transparent'}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        <IconComp className={`w-5 h-5 ${isWarning ? 'text-[#EF4444]' : 'text-[#3B82F6]'}`} />
                        {isWarning && (
                          <span className="flex items-center gap-1 text-[10px] uppercase font-bold tracking-wider text-[#EF4444] bg-[#EF4444]/10 px-2 py-0.5 rounded-full border border-[#EF4444]/20">
                            <AlertTriangle className="w-3 h-3 flex-shrink-0" /> Moratorium Zone
                          </span>
                        )}
                      </div>
                    </div>
